...
public class EmojiDao {
    ...
    private EmojiDao(Context context){
        try {
            path = CopySqliteFileFromRawToDatabases(context,"emoji.db",context.getExternalCacheDir().getPath());
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    public List<EmojiBean> getEmojiBean(){
        ...
        Cursor cursor = null;
        try {
            cursor = db.query("emoji", new String[]{"unicodeInt","_id"}, null, null, null, null, null);
     ...
     * @return 存储数据库的地址
     */
    public static String CopySqliteFileFromRawToDatabases(Context context, String SqliteFileName,String path) throws IOException {
        // 第一次运行应用程序时，加载数据库到data/data/当前包的名称/database/<db_name>
        //复制的话这里需要换成自己项目的包名
        File dir = new File(path + "/databases");
        ...
        if (!file.exists()) {
            try {
                file.createNewFile();
                inputStream = context.getAssets().open(SqliteFileName);
