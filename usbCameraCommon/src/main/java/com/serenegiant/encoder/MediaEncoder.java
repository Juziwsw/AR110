...
//		android.os.Process.setThreadPriority(android.os.Process.THREAD_PRIORITY_URGENT_AUDIO);
        ...
        while (isRunning) {
			if (DEBUG) Log.d(TAG, "isRunning = mRequestDrain="+mRequestDrain);
        	synchronized (mSync) {
			...
			if (DEBUG) Log.d(TAG, "localRequestDrain ="+localRequestDrain);
	        if (localRequestDrain) {
	        	drain();
	        } else {
	        	synchronized (mSync) {
...
LOOP:	while (mIsCapturing) {
            ...
            if (encoderStatus == MediaCodec.INFO_TRY_AGAIN_LATER) {
				...
				if (DEBUG) Log.v(TAG, "else encoderStatus:"+encoderStatus);
